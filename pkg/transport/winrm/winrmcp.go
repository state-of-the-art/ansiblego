...
// functions from https://github.com/packer-community/winrmcp/blob/master/winrmcp/cp.go
func doCopy(client *winrm.Client, parallel, max_chunks_per_shell int, in io.Reader, to_path string) error {
	...
	err = uploadContent(client, max_chunks_per_shell, "%TEMP%\\"+tempFile, in)
	if err != nil {
		return fmt.Errorf("Error uploading file to %s: %v", tempPath, err)
	}

	log.Printf("Moving file from %s to %s", tempPath, to_path)

	err = restoreContent(client, tempPath, to_path)
	if err != nil {
		return fmt.Errorf("Error restoring file from %s to %s: %v", tempPath, to_path, err)
	}
